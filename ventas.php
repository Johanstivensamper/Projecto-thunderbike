    ...
    <div class="container">
        ...
        if (is_array($result_ventas) && count($result_ventas) > 0) {
            foreach ($result_ventas as $row) {
                ...
                echo "<td>
                        <a href='editar_venta.php?id=" . htmlspecialchars($row["id"]) . "' class='btn'>Editar</a>
                        <a href='eliminar_venta.php?id=" . htmlspecialchars($row["id"]) . "' class='btn' onclick='return confirm(\"¿Estás seguro de que deseas eliminar esta venta?\");'>Eliminar</a>
                    </td>";
                echo "</tr>";
            }
        } else {
